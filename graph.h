...
public:
  ...
  virtual void set_edge(int v1, double edge_label, int v2) = 0;
  ...
  virtual bool get_edge(int v1, int v2, double& edge) const = 0; 

  // remove edge between v1 and v2 if the edge exists, return true if the edge was found and removed
  ...
  //----------------------------------------------------------------------
  bool shortest_path_length(bool dir, int src, int dst, std::list<int>& path) const;
...
//----------------------------------------------------------------------
  ...
  //
  // Returns: True if graph is connected and communites have been found
  //          False if the graph is disconnected and communites can not be found
  //----------------------------------------------------------------------


  bool girvan_newman(Graph& community_graph);

  // Helper alogrithm for Girvan-Newman algorith
  // Used to find the number of shortest paths from a source vertex to all other vertices
  // returns the number of shortest paths sorted by distance from the src
  // Note: There are only multiple shortest paths when they are the same size 
  // Implemented via a modified BFS 
  void number_of_shortest_paths(int src, Map& num_shortest_paths, Map& distance, std::list<std::pair<int,std::pair<int,int>>>& ordered_by_distance)const;

  // Helper alogrithm for Girvan-Newman algorith
  // Used to find the edges scores for calculating community
  // returns a vector of edges that hold the edge and score (u, w, v)
  // Implemented via a modified BFS 
  ...
  void edge_scores(std::vector<std::tuple<int,double,int>>& edges, Map& num_shortest_paths, Map& distance, std::list<std::pair<int,std::pair<int,int>>>& ordered_by_distance)const;
